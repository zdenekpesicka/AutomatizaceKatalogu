...
| Téma | Pravidlo |
|---|---|
| **Verze schématu** | Aktuální je **1.3.0**. Odsouhlasené s vývojářem bylo 1.0.0; 1.1.0 přidalo `sluzby[].zarizeni`, 1.2.0 `sluzby[].oboryPece`, 1.3.0 kategorii `poradenstvi`. Žádná z nich neodebrala pole ani nezměnila typ. **Přírůstková pole 1.1.0 a 1.2.0 zatím nebyla vývojáři ohlášena**, i když jsou už v produkčních datech; podle 4.2 se dopředu hlásí jen nekompatibilní změna, takže jde o informaci, ne o porušení pravidla. **U 1.3.0 to neplatí a ohlásit se musí:** rozšíření výčtu v `kategorie[]` ze čtyř hodnot na pět je jediná dosavadní změna, která může rozbít čtenáře — validace 1.3.0 dat proti staršímu schématu selže a stejně tak kód, který si čtveřici zapsal natvrdo. Vývojář zatím katalog neintegroval, takže je to v tuto chvíli informace, ne oprava; po spuštění webu už by táž změna byla důvod k 2.0.0. Verze je na třech místech a musí sedět: `VERZE_SCHEMATU` v `build_katalog.py`, `verzeSchematu` v `meta.json` a v `katalog.json`. `data/ukazka.json` je zmrazený na 1.0.0 záměrně, neaktualizuje se. |
