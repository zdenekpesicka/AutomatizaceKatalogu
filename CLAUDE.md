...
| **Připnuté akce cílí na Node.js 20, runner je nutí na Node 24.** | Vypsáno jako anotace v běhu `34350398991` (9. 9. 2026) u všech tří akcí: `actions/checkout`, `actions/setup-python` i `actions/cache`. Dnes to nic nerozbíjí, běh je zelený. Je to ale přímý důsledek připnutí na commit SHA (8.3, řádek o odolnosti stahování): připnutá verze se sama neposune, takže až GitHub podporu Node 20 ukončí, přestanou akce fungovat a SHA se budou muset povýšit ručně. Vědomá cena za to, že přepsaná značka `v4`/`v5` nemůže změnit, co v běhu importu poběží. Řeší se povýšením SHA na novější vydání akcí, ne návratem k pohyblivým značkám. |
| **Odkazy v `$id` a v README ukazují na dnešní účet.** | `schema/katalog.schema.json` má `$id` `https://raw.githubusercontent.com/zdenekpesicka/AutomatizaceKatalogu/…` a tabulka odběru dat v `README.md` uvádí tytéž raw adresy. Dokud je repozitář na tomto účtu, jsou správné. **Při převodu na účet klienta (etapa 5, bod 3) se musí přepsat obojí** a nová adresa oznámit vývojáři, protože podle ní odebírá data. GitHub sice na starou adresu přesměrovává, ale spoléhat se na přesměrování u identifikátoru schématu a u produkčního odběru dat není namístě. Zapsáno sem, protože jde o jedinou položku, která je dnes v pořádku a převodem se stane nesprávnou. |
| **Záložky Bezpečí a Zdraví na webu popisují produkty, kategorie v datech popisují služby.** | Definice záložek, podle kterých se zatřídění stavělo 11. 9. 2026, znějí u Bezpečí „chytré hodinky s SOS tlačítkem, GPS lokátory, dávkovače léků, senzory pohybu“ a u Zdraví „klinická výživa, doplňky stravy, inkontinenční pomůcky, krabičkové stravování, domácí diagnostické testy“. To jsou zboží, které v RPSS ani NRPZS není a být nemůže — oba registry evidují poskytovatele služeb. Naše stejnojmenné kategorie obsahují něco jiného: `zdravi` je **890 míst** (762 služeb domácí zdravotní péče, 55 LDN, 55 nemocnic následné péče, 30 hospiců, 28 rehabilitačních ústavů), `bezpeci` **35 míst** (tísňová péče a krizová pomoc). Z toho jen tísňová péče (24 míst) má v registru obsah odpovídající definici záložky — je to registrovaná služba, která SOS tlačítko a dispečink provozuje. **Riziko je záměna podle názvu:** kdyby vývojář naplnil záložku Zdraví kategorií `zdravi`, objeví se mezi produkty lůžková zařízení. Změřeno nad výstupem 1.4.0: z 890 míst v `zdravi` jich **126 nemá jinou kategorii** — 48 LDN, 39 nemocnic následné péče, 26 rehabilitačních ústavů a 15 lůžkových hospiců. Právě tato lůžková zařízení by se bez záložky Zdraví nezobrazila nikde; zbylých 764 míst je vidět i v `terenni`, protože jde o domácí zdravotní péči, která od 1.4.0 patří do obou kategorií. Před přestavbou byl ten počet 651 z 890. Rozhodnuto 11. 9. 2026 ponechat beze změny, případné skrytí je otázka zobrazení. Otevřené zůstává, jestli lůžková zařízení dostanou vlastní záložku, nebo se z webu vypustí. |
| **54 míst s denní docházkovou péčí nemá záložku.** | Denní stacionáře (`DruhSocialniSluzby/10`, 135 míst) a centra denních služeb (`/9`, 32 míst) jsou v registru výhradně ambulantní, takže do `terenni` po přestavbě v 1.4.0 nepatří, a žádná jiná záložka na ně nesedí. Rozhodnuto 11. 9. 2026 nemapovat je nikam. Důsledek: **54 míst se nezobrazí na webu vůbec**, protože nemají na téže adrese jinou zařazenou službu (42 s denními stacionáři, 12 s centry denních služeb); zbylých 113 míst obou druhů je vidět pod jinou kategorií. Data zůstávají ve výstupu v úplné podobě, takže případné doplnění záložky je jen změna v `config/kategorie-mapovani.json`, ne nový sběr. |
...
| **Zpoždění plánovaných běhů je v řádu hodin, ne minut.** | Sekce 5.4 uvádí „zpoždění 5 až 30 minut je běžné“. Změřeno 9. 9. 2026 na všech pěti dosavadních plánovaných bězích (`0 4 * * *` UTC): skutečné starty 07:55, 08:12, 08:25, 08:28 a 08:44 UTC, tedy zpoždění 236 až 285 minut, a všechny běhy skončily úspěchem. Není to porucha na naší straně — GitHub naplánované běhy řadí do fronty podle vytížení a čas v cronu je nejdřívější možný start, ne závazek. Provozně to nevadí: MPSV publikuje jednou denně a katalog nemá požadavek na hodinu vydání. **Důsledek pro dokumentaci:** README i `data/dokumentace-rozhrani.md` konkrétní rozpětí neuvádějí a odkazují příjemce na `hashKatalogu`, ne na očekávanou hodinu. Konkrétní čísla se do nich záměrně nepíšou, protože závisejí na vytížení GitHubu a zastaraly by. Zadání 5.4 se podle pravidla 0.1 nepřepisuje. |
...
| Téma | Pravidlo |
|---|---|
| **Verze schématu** | Aktuální je **1.4.0**. Odsouhlasené s vývojářem bylo 1.0.0; 1.1.0 přidalo `sluzby[].zarizeni`, 1.2.0 `sluzby[].oboryPece`, 1.3.0 kategorii `poradenstvi`, 1.4.0 kategorii `pecovatelske` a přestavbu zatřídění. Žádná z nich neodebrala pole ani nezměnila typ. **Rozšíření výčtu v `kategorie[]` se podle 4.2 hlásí dopředu** (na pět hodnot v 1.3.0, na šest v 1.4.0), na rozdíl od přírůstkových polí v 1.1.0 a 1.2.0. Schéma leží v témže repozitáři jako data a odebírá se ze stejné adresy, takže příjemce dostane nový výčet spolu s daty. **1.4.0 je první verze, ve které místo o kategorii i přišlo**, ne jen získalo — 121 míst vypadlo z `terenni` a 54 míst zůstalo bez kategorie úplně. Struktura se tím nemění a čtení nespadne, ale dřív zobrazované místo se zobrazovat přestane; to je důvod, proč se hlásí i přesun, ne jen přírůstek. Po spuštění webu už by táž změna byla důvod k 2.0.0. Verze je na třech místech a musí sedět: `VERZE_SCHEMATU` v `build_katalog.py`, `verzeSchematu` v `meta.json` a v `katalog.json`. `data/ukazka.json` je zmrazený na 1.0.0 záměrně, neaktualizuje se. |
...
| **Sekundární druh zařízení ÚZIS se záměrně nečte** | `ZZ_druh_nazev_sekundarni` je vyplněný u 1 073 z 40 870 řádků a relevance se posuzuje jen podle `ZZ_druh_nazev`. Vypadá to jako opomenutí, není. Změřeno 8. 9. 2026: relevantní sekundární druh má 20 vyřazených řádků (14× domácí zdravotní péče, 4× LDN, 2× zdravotní péče v ústavech) a jejich plošné zařazení by přidalo 14 nových míst a 6 připojilo ke stávajícím. Mezi 14 novými by byly Psychiatrická nemocnice Bohnice, Nemocnice Na Františku, Nemocnice sv. Zdislavy, Sanatorium Jablunkov, dvě zdravotnické dopravní služby (Royal Rangers, Richard Balcar), služba přepravy pacientů neodkladné péče (EMA Emergency) a pět samostatných ordinací; mezi 6 připojenými je i lékárna (Domov pro seniory Dobrá Voda, Lékárna Pod Lesem) — tedy přesně množina, kterou z týchž důvodů odmítá už filtr hospiců v `uzis.py` (celé nemocnice a soukromé ordinace jen proto, že mají relevantní licenci). Skutečně seniorské jsou z těch dvaceti tři: Světlo do Vašich domovů LuNA, Centrum zdravotní a sociální péče Liberec a Charita Polička. Poměr 3 ku 17 rozhodl; kdyby se sloupec začal číst, muselo by k němu přibýt další rozlišovací pravidlo, které registr neposkytuje (pravidlo 0.2). |
...
| **Ověření pojistek (kontrolní bod etapy 4)** | Změřeno 7. 9. 2026 na plných datech, nad výstupem s 2 912 místy. **(a) Poškozený zdroj:** v `_cache/rpss.json` přepsán `druhSocialniSluzby` prvního záznamu z objektu na číslo. Běh spadl na validaci proti `rpss.schema.json` (`13 is not of type 'object'`) dřív, než se cokoli zpracovalo; `git status data/` prázdný. **(b) Prahová kontrola:** v `meta.json` snížen `pocetMist` na 2 000. Běh vypsal `POJISTKA: pocet mist se zmenil o 45.6% (2000 -> 2912), limit je 5%. NEPUBLIKUJI` a skončil s návratovým kódem 1, tedy selháním workflow, které vyvolá notifikaci (že notifikace opravdu dorazí, se ověřovalo zvlášť — viz řádek o doručení notifikace níže). `katalog.json` i `zmeny.json` mají po běhu nezměněný kontrolní součet. **(c) Ruční přepínač:** týž stav s `--zamerna-velka-zmena` prošel jako varování místo odmítnutí, ale nic nezapsal, protože obsah katalogu byl shodný — přepínač povoluje překročení prahu, nevynucuje zápis. **(d) Kontrola duplicitních ID** se testovala sama v ostrém provozu: do 4. 9. 2026 kolidovala 3 ID na 8 místech a běh je odmítl publikovat. Validace vlastního výstupu proti `katalog.schema.json` běží při každém běhu a je v logu (`Validuji vystup proti schematu... OK, 0 chyb`). |
| **Pořadí `mista[]` a `sluzby[]` se nesrovnává** | Pořadí ve výstupu vzniká průchodem přes zdrojové soubory a kopíruje pořadí záznamů v nich; `build_katalog.py` ho před zápisem nesrovnává. Stojí to na očekávání, že registry exportují ve stabilním pořadí, a to očekávání je doložené: jediný dosavadní automatický běh (`e2a5523`, 4. 9. 2026) stahoval `rpss.json` i `nrpzs.csv` nanovo a relativní pořadí všech 3 245 společných míst (3 252 před i po) zůstalo proti předchozí verzi zachované, diff se dotkl 1,2 % řádků souboru. Garance to ale není a nepředstírá se: ani jeden zdroj není seřazený podle žádného klíče (`portalId` v RPSS jde 4951, 2624, 4400, 4831, 6275…), a ověřeno přeházením pořadí na plných datech vznikne z téže množiny míst jiné pořadí, tedy jiný hash. **Kdyby k tomu došlo, projeví se to jedním commitem s diffem přes celý soubor, u kterého `zmeny.json` hlásí 0 přidaných, 0 změněných a 0 odebraných** — porovnává se podle `id`, takže pouhé přeskupení v něm není vidět. Data zůstávají správná, jde o šum v historii, ne o vadu obsahu. Příjemce se proto nemá na pořadí prvků spoléhat (zapsáno v `data/dokumentace-rozhrani.md`). Nesrovnává se proto, že by to ošetřovalo stav, který nenastal. Dostupné měření ukazuje stabilní pořadí a přibývání nových zařízení na něm nic nemění — nový záznam se vloží na svou pozici, vzájemné pořadí ostatních zůstává. Obrana proti přeskupení by tak stála na domněnce, ne na datech. Kdyby k němu došlo, řešením je seřadit `mista[]` a `sluzby[]` podle `id` před zápisem. Jediná výjimka, kde se dnes řadí: u míst čistě z ÚZIS jsou `sluzby[]` seřazené podle `ZZ_misto_poskytovani_ID` (`build_katalog.py:513`), a to ne kvůli stabilitě výstupu, ale proto, že první řádek určuje název a adresu místa — bez seřazení by je vybíral pořadím v CSV. |
| **Pětina Domovů jsou pobytové odlehčovací služby** | Rozhoduje forma, ne druh (4.3), takže pobytová odlehčovací služba je v Domovech vedle domovů pro seniory. Změřeno 9. 9. 2026: z 813 míst v Domovech je **122 tam výhradně kvůli ní**, tedy bez domova pro seniory, domova se zvláštním režimem, týdenního stacionáře nebo chráněného bydlení na tomtéž adresním bodě. Vypadá to jako vada zařazení, není: **registrovanou kapacitu má všech 122** — 121 v lůžkách (1 824 lůžek, 1 až 54 na místo), `misto-9164898` (Centrum služeb pro zdravotně postižené Žatec) ji má registrovanou v typu `klient` (14). Nejde tedy o kanceláře terénní služby, ale o skutečná lůžka na přechodný pobyt, což je přesně to, co hledá pečující člověk shánějící úlevu. 87 ze 122 je zároveň v Terénních službách, protože táž organizace na téže adrese provozuje i terénní službu. Vyřazovat je z Domovů by znamenalo rozhodovat podle druhu proti výslovnému pravidlu 4.3. Rozlišit je lze podle `sluzby[].druhSluzby.kod` = `DruhSocialniSluzby/8`; zapsáno pro příjemce v `data/dokumentace-rozhrani.md`, sekce Kategorie. |
| **Příznak `poskytujeZdravotniPeci` se připojuje jen k místům z MPSV** | Vypadá to jako mezera — místo, které je v katalogu jen z ÚZIS, příznak nedostane, i když na jeho adresním bodě zdravotnická licence domova existuje. Změřeno 9. 9. 2026 na plných datech: ze 783 řádků druhu „Zdravotní péče v ústavech sociální p.“ se 438 připojí k místu s MPSV službou, **4 řádky sdílejí adresní bod s místem, které je v katalogu jen z ÚZIS** (jsou to jen 3 různá místa, `misto-82027471` je tam dvakrát), a 341 nemá na svém adresním bodě místo žádné. Těch 341 + 4 = 345 zahozených řádků souhlasí s číslem v 8.2. Rozšířit připojování i na ÚZIS místa by bylo věcně chybné: ty 4 řádky jsou 2× „Léčebna pro dlouhodobě nemocné (LDN)“ a 2× „Domácí zdravotní péče“, tedy zdravotnická zařízení, ne domovy. Příznak znamená „tento domov má i ošetřovatelský úsek“ a u LDN nedává smysl — LDN je zdravotnické zařízení celá. Chování je proto správné a plyne z toho, že se `places` v okamžiku připojování příznaku (`build_katalog.py:425`) skládá jen z MPSV míst; ÚZIS místa vznikají až za tím. |
| **Kontrola stáří zdrojových dat** | Od 9. 9. 2026 běží před vším ostatním: když je kterýkoli ze tří snapshotů v `_cache/*_meta.json` starší než `PRAH_STARI_ZDROJU = 50` dnů, běh skončí návratovým kódem 1 a nic nepublikuje. Pokrývá poruchu, kterou klíč cache pokrýt nemůže — trvale selhávající sondu, kdy se běh záměrně drží souborů na disku a končí zeleně (viz řádek o klíči cache výše). Práh je odvozený z měření, ne odhadnutý: 9. 9. 2026 byl MPSV starý 1 den, ÚZIS 8 a RÚIAN 9. Práh je nastavený podle měsíčních zdrojů (ÚZIS a RÚIAN), u kterých je zdravé maximum kolem 32 dnů; MPSV vychází denně a k prahu se nepřiblíží, ale kontrole podléhá také. 50 dnů nechává rezervu na opožděné vydání a 50 nechává rezervu na opožděné vydání, ale zachytí zaseknutí do dvou týdnů. Čte se z `_cache/`, nikdy z publikovaného `meta.json` — to se přepisuje jen při změně katalogu (viz řádek o `datumZdrojovychDat`), takže by kontrola měřila stáří publikace místo stáří zdroje. Ruční překlenutí `--zastarale-zdroje-ok` pro běh nad záměrně starým `_cache/`; workflow ho nepředává. Ověřeno: zestárlý snapshot → návratový kód 1 a `data/` beze změny, s přepínačem → varování a běh pokračuje. **Náhradní datum kontrolu neumlčí.** Když `stahni_zdroje.py` datum zdroje nezjistí (výpadek `Last-Modified`, změněný název souboru ČÚZK), dosadí dnešní — to je vždy čerstvé, takže by práh nikdy nepřekročilo a kontrola by mlčela právě v případě, kdy o stáří zdroje nevíme nic. Od 9. 9. 2026 proto meta soubory v `_cache/` nesou navíc klíč `nahradniDatum` se seznamem takových zdrojů a kontrola je bere jako závadu bez ohledu na práh. Překlenuje se toutéž `--zastarale-zdroje-ok`. |
| **Selhání sondy na verze zdrojů je vidět na stránce běhu** | Sonda záměrně nikdy neshodí běh, aby výpadek ČÚZK nezastavil i import MPSV — vypíše varování na stderr, vrátí náhradní klíč a skončí s kódem 0. Důsledek je, že běh nad poslední uloženou verzí vypadá v seznamu Actions stejně zeleně jako běh nad čerstvými daty a varování zapadne v logu. Od 9. 9. 2026 proto `import.yml` obsahuje krok podmíněný `steps.verze.outputs.klic == 'uzis-ruian-neurceno'`, který vypíše `::warning::` a GitHub ho zobrazí jako anotaci nad výpisem běhu. **Anotace se nevypisuje přímo ze sondy**, protože její stdout je v tom kroku přesměrovaný do `$GITHUB_OUTPUT` — `::warning::` by tedy skončilo mezi výstupy kroku, ne v logu. Anotace je upozornění, ne pojistka; tou zůstává kontrola stáří zdrojů, která začne selhávat, jakmile výpadek přeroste 50 dnů. |
| **Prahová kontrola na místa bez souřadnic** | Práh z 5.2 bodu 3 hlídá počet míst, ale useknutý RÚIAN by mu unikl: míst by zůstal stejný počet, jen by tiše přišla o souřadnice a katalog by šel na web s prázdnou mapou. Od 9. 9. 2026 proto běží druhý práh — nárůst o víc než `PRAH_BEZ_SOURADNIC = 30` míst proti poslední publikované verzi znamená nepublikovat. Hodnota je změřená přes všech deset historických verzí `katalog.json`: počet míst bez souřadnic se držel mezi 110 a 112, tedy rozptyl ±2, a nepohnul se ani při výměně RÚIAN snapshotu z 31. 7. za 31. 8.; 30 je proti tomu asi patnáctinásobná rezerva. Referenční hodnota je nové pole `pocetMistBezSouradnic` v `meta.json`. To se ale zapisuje jen při změně katalogu (viz řádek o `datumZdrojovychDat`), takže v publikovaných datech chybí tak dlouho, dokud nepřijde první změna obsahu — a přesně v tom okně by pojistka mlčela, tedy v době, kdy je nejčerstvější a nejméně prověřená. Od 9. 9. 2026 se proto reference při chybějícím poli dopočítá přímo z posledního publikovaného `data/katalog.json` (`referencni_pocet_bez_souradnic` v `build_katalog.py`), což je zdroj té hodnoty tak jako tak. `None` a přeskočení kontroly zbývá jediný stav: úplně první běh, kdy žádný publikovaný katalog není a není proti čemu porovnávat. Překlenuje se toutéž `--zamerna-velka-zmena` jako práh na počet míst — obojí jsou pojistky proti rozbitému zdroji, ne proti záměrné změně zpracování. Ověřeno: umělý skok z 10 na 110 (+100) → návratový kód 1, s přepínačem → varování a nic nezapsáno. |
| **Záložka Poradenství a doplnění zatřídění (11. 9. 2026)** | Otevřená otázka „324 míst nemá žádnou kategorii" je uzavřená, rozhodnuto 11. 9. 2026. Mapovací tabulka 4.3 pokrývala 10 z 21 druhů sociálních služeb vyskytujících se u seniorů; doplnilo se jich sedm. **Do `terenni`:** sociálně aktivizační služby pro seniory (`/29`), sociální rehabilitace (`/33`), centra denních služeb (`/9`), průvodcovské a předčitatelské služby (`/6`) — 1 280 → 1 381 míst. **Do `bezpeci`:** telefonická krizová pomoc (`/18`) a krizová pomoc (`/23`) vedle dosavadní tísňové péče — 24 → 35. **Nová kategorie `poradenstvi`:** odborné sociální poradenství (`/1`), 374 míst, z toho 217 bez jiné kategorie. Vlastní kategorie proto, že to není péče, ale informace (poradny pro pečující, občanské poradny, půjčovny kompenzačních pomůcek), a do Terénních služeb vedle pečovatelské služby nepatří; zároveň je to u 237 z těch 324 míst jediná služba, takže bez ní by zůstala neviditelná. Pobytová forma se u žádného ze sedmi druhů nemapuje — pobytově se neposkytují. **Nezařazené zůstávají čtyři druhy** (sociálně terapeutické dílny, domovy pro OZP, tlumočnické služby, sociálně aktivizační služby pro rodiny s dětmi) na **3 místech**: `misto-13000209` (Liblín), `misto-25253867` (Mačkov), `misto-3250377` (Ostrava). Nejsou to seniorské služby, v katalogu jsou jen proto, že mají mezi cílovými skupinami i seniory. **Změřeno nad přechodovým během:** 468 změněných míst, 1 přidané, 6 odebraných, 2 912 → 2 907 míst a 4 007 → 4 003 služeb. **Za kategorie z toho může jen 464 změněných míst**, u kterých se lišilo výhradně pole `kategorie` a vždy jen přibytím hodnoty; zbylé 4 změněné, přidané i všech 6 odebraných je běžný denní pohyb MPSV, protože přechodový běh nutně stavěl nad novějším snapshotem (10. 9. 2026) než poslední publikovaná verze. Odebraná: `misto-10745904`, `misto-10745912` a `misto-10746951` (Hostim, `mpsv-388`), `misto-28418671` a `misto-80048866` (`mpsv-7166`, registrace končí 11. 9. 2026) a `misto-9599134` (Havlíčkův Brod, `mpsv-7211`); přidané `misto-5992974` (Olovnice). **Dvě čísla v `meta.json` proto klesla navzdory tomu, že kategorie jen přibývaly:** `domovy` 813 → 810 a `pocetMistSPoskytovanimZdravotniPece` 430 → 429, obojí odchodem míst výše, ne přetříděním. Obě prahové pojistky prošly bez přepínače, počet míst ani míst bez souřadnic (110 → 109) se skokově nezměnil. Kategorie `poradenstvi` je nad rámec tabulky v 4.3, která se podle pravidla 0.1 nepřepisuje. Změna je v `config/kategorie-mapovani.json` a ve výčtu ve schématu; kód se měnil jen v jednom místě — seznam kategorií pro `pocetMistPodleKategorie` v `build_katalog.py` byl vypsaný natvrdo, takže by nová kategorie v `meta.json` tiše chyběla, a nově se odvozuje z konfigurace. |
| **Přestavba zatřídění podle definic záložek webu (11. 9. 2026, schéma 1.4.0)** | Ke každé záložce webu existuje výčet „co sem spadá“ a zatřídění se podle nich přestavělo. Dvě pravidla se změnila, jedna kategorie přibyla. **(a) `terenni` je nově jen to, co za klientem dojede.** Do 1.3.0 se do ní mapovala i forma `amb`, tedy provozovna, kam klient dochází. Nově se mapuje jen forma `ter`, a ze zdroje ÚZIS nově i druh „Domácí zdravotní péče“ — jediný druh ÚZIS ve dvou kategoriích zároveň (`zdravi` i `terenni`). Mobilní hospice jsou v tom zahrnuté, protože je registr vede jako domácí zdravotní péči, ne jako „Hospic“ (3.2). Změřeno: 1 381 → **1 818 míst**, z toho +558 s domácí zdravotní péčí (celkem jich ji má 734, z toho 526 nemá žádnou sociální službu) a −121 míst, která měla jen ambulantní formu. **(b) Nová kategorie `pecovatelske`, 1 328 míst:** pečovatelská služba (`/4`), odborné sociální poradenství (`/1`) a sociálně aktivizační služby pro seniory (`/29`), v terénní i ambulantní formě. Terénní forma `/4` a `/29` je proto zároveň v `terenni` i `pecovatelske` — pravidlo zní, že služba, která jezdí k lidem, může být v obou. Jen 22 z 1 328 míst nemá jinou kategorii. **(c) `poradenstvi` ztratilo výhradní místa.** V 1.3.0 mělo 217 míst ze 374, která by bez páté záložky nebyla vidět nikde; od 1.4.0 je `/1` zároveň v `pecovatelske`, takže výhradních míst je **0** a všech 374 je vidět i pod Pečovatelskými službami. **(d) Denní stacionáře (`/10`) a centra denních služeb (`/9`) se vyřadily z mapování úplně.** Oba druhy jsou v datech výhradně ambulantní (135 a 32 míst), takže do `terenni` podle nové definice nepatří, a záložku pro denní docházkovou péči web nemá. Rozhodnuto 11. 9. 2026 nemapovat je nikam. Důsledek: míst bez kategorie je **57 místo 3** — 42 s denními stacionáři, 12 s centry denních služeb a původní 3 (`misto-13000209`, `misto-25253867`, `misto-3250377`). Pět z těch 54 je zároveň bez adresy, takže v dokumentaci rozhraní klesl počet míst bez adresy, která mají kategorii, z 36 na 31. **(e) Krizová pomoc zůstává v `bezpeci`** vedle tísňové péče, ačkoli definice záložky Bezpečí mluví o zařízeních (SOS tlačítka, GPS lokátory, dávkovače léků) a `/18` a `/23` jsou psychosociální intervence; ponecháno beze změny. `domovy` (810), `bezpeci` (35) a `zdravi` (890) se nezměnily. **Změřeno nad přechodovým během:** 1 961 změněných míst, 0 přidaných, 0 odebraných, a **u všech 1 961 se liší výhradně pole `kategorie`** — ověřeno porovnáním celých objektů míst proti předchozí publikované verzi. Počet míst (2 907), služeb (4 003), míst bez souřadnic (109) ani `pocetMistSPoskytovanimZdravotniPece` (429) se nezměnily, obě prahové pojistky prošly bez přepínače. Změna je celá v `config/kategorie-mapovani.json` a ve výčtu ve schématu; v kódu se měnila jen konstanta `VERZE_SCHEMATU`, protože seznam kategorií pro `meta.json` se od 1.3.0 odvozuje z konfigurace. Tabulka v 4.3 se podle pravidla 0.1 nepřepisuje. |
| **Doručení notifikace o selhání je ověřené** | Etapa 5 bod 4 předepisuje ověřit upozornění úmyslným selháním běhu. Provedeno 9. 9. 2026, tedy ještě před předáním, aby se vada v doručování nenašla až u klienta. Na dočasné větvi `test-notifikace` přibyl hned za `checkout` krok `exit 1` a běh `34354531689` (13:01 UTC, `workflow_dispatch`) skončil selháním na třetím kroku; kroky 4 až 14 se přeskočily, takže se nic nestahovalo a `Commituj zmeny` neproběhl — `main` zůstal na `324c0e5` a `data/` beze změny. E-mail od GitHubu dorazil. Nastavení příjemce v Settings → Notifications → System → Actions bylo `On GitHub, Email` s omezením `Only notify for failed workflows`, tedy stav, který zadání 5.4 předepisuje; zelené denní běhy proto neposílají nic. Větev byla po ověření smazána, záznam běhu v Actions zůstává. **Ověření platí pro dnešního vlastníka repozitáře.** Adresátem je autor workflow, respektive ten, kdo naposledy měnil `cron`, takže se převodem na účet klienta nepřenese samo — postup předání je v etapě 5 bodu 4 a musí se ověřit znovu, u klienta. |
| **Validace výstupu kontroluje i `format`** | `jsonschema.validate` bez `format_checker` bere `"format": "date"` jen jako anotaci a nekontroluje ho. Schéma přitom `date` používá u `datumPoskytovaniOd` a `datumPoskytovaniDo`, takže by jimi prošel libovolný řetězec. Od 9. 9. 2026 se validace výstupu volá s `format_checker=jsonschema.Draft7Validator.FORMAT_CHECKER`. **U validace zdrojového `rpss.json` se záměrně nezapíná:** kontroluje se cizí soubor podle cizího schématu a zpřísnění by mohlo shodit běh na formátu, který nepoužíváme a který registr sám nedodržuje. U vlastního výstupu je to naopak žádoucí — schéma je naše a příjemce se na ně spoléhá. |
| **Testy nad zpracováním běží před stahováním** | `tests/test_zpracovani.py`, 25 testů. V obou workflow běží hned po instalaci závislostí a před prvním sáhnutím na zdroje (konkrétně jako 4. krok: checkout, setup-python, `pip install`, testy). Pokrývají místa, kde už jednou vznikla chyba v datech nebo kam ji zdroje aktivně zvou: prohozené pořadí ve WKT z ÚZIS (3.1) včetně odmítnutí bodu mimo ČR, rozpad vícehodnotového `ZZ_obor_pece` (3.3), slučování registrací podle `portalId` včetně případu s koncovou mezerou (8.3), datum v klíči cache, kontrolu odkazu z ATOM feedu a odstup mezi pokusy ve `fetch.get_bytes` i `fetch.last_modified` (`requests.get`, `requests.head` i `time.sleep` jsou nahrazené, síť se nepoužívá). Nejsou to testy „pro pokrytí“ — každý odpovídá konkrétní vlastnosti zdroje nebo poruše, která už jednou nastala. Nesahají na síť, proto mohou běžet ještě před stažením: když je rozbité zpracování, nemá smysl tahat 190 MB dat. Na `_cache/` sahají jen nepřímo — import `build_katalog` spustí `nacti_datum_zdrojovych_dat()`, která meta soubory přečte, když existují; když chybí, jen varuje na stderr a dosadí dnešní datum, takže v CI běží nad neexistujícím `_cache/` bez potíží. Do 9. 9. 2026 docstring testů i komentáře v obou workflow tvrdily „nesahají na síť ani na `_cache/`“, což bylo o jedno slovo silnější, než jak to je. `pytest` je v odděleném `import/requirements-dev.txt`, který k `requirements.txt` přidává jen jeho. **Obě workflow instalují `requirements-dev.txt`**, protože testy v nich běží — dělení souborů tedy neslouží CI, ale tomu, kdo si chce postavit katalog bez testovací závislosti. |
| **Odolnost stahování a kontrola odkazu z ATOM feedu** | Čtyři úpravy z auditu 9. 9. 2026. **(a) Odstup mezi pokusy:** `fetch.py` opakoval pokusy bez čekání, takže všech pět proběhlo během milisekund — dřív, než stačil odeznít i ten nejkratší výpadek zdroje, a retry jen zopakoval tutéž chybu. Nově 2, 4, 8, 16 s se stropem 16, tedy nejvýš 30 s čekání na soubor a hluboko pod limitem jobu (15 minut u denního běhu, 30 u ručního). Odstup i strop hlídá test — první verze komentáře tvrdila „pod 30 s“, zatímco součet je přesně 30, a test to odhalil. **(a2) ATOM feed měl retry chybějící úplně:** oba dotazy na feed ČÚZK (`stahni_zdroje.py`, ve stahování i v sondě) volaly `requests.get` napřímo, takže jediný neúspěšný požadavek shodil celé stažení ÚZIS a RÚIAN, případně poslal denní běh na náhradní klíč cache — a ten stojí 92 MB stažení navíc. Nově jdou přes `fetch.get_bytes` se stejným odstupem jako `download`; streamování ani `Range` u pár kB smysl nemají, odolnost proti krátkému výpadku ano. **(b) Kontrola odkazu z feedu ČÚZK:** `<id>` je jen řetězec a bez kontroly by přesměrování nebo změna feedu poslaly stahování kamkoli. `resolve_download_url` proto pinuje host `vdp.cuzk.gov.cz`, vyžaduje HTTPS a tvar `RRRRMMDD_OB_ADR_csv.zip`, a při chybějícím `<entry>` nebo `<id>` selže se srozumitelnou hláškou místo tichého pádu na náhradní klíč cache. Z názvu souboru se totiž čte datum do klíče cache. **(c) Actions připnuté na commit SHA** místo pohyblivých značek `v4`/`v5`, aby přepsaná značka nemohla změnit, co v běhu importu poběží. |
| **Náhradní datum se nesmí uložit do cache** | Nalezeno při auditu 9. 9. 2026, jediná provozní past, která zbývala. `last_modified()` neměla retry a selhání polykala, takže **jediný neúspěšný HEAD na ÚZIS během stahování** zapsal do `_cache/uzis_ruian_meta.json` dosazené dnešní datum a `nahradniDatum: ["uzis"]`. Sama o sobě je to zamýšlená a hlasitá porucha — kontrola stáří zdrojů na náhradní datum spadne a nic se nepublikuje. Uvízla ale na tom, **odkud se bere klíč cache**: klíč pochází ze sondy, která proběhla o pár minut dřív a uspěla, takže je platný. Otrávený meta soubor se pod ním uložil a každý další denní běh na něj trefil přesnou shodu klíče, tedy nestahoval, načetl totéž náhradní datum a spadl znovu — až do nejbližšího nového vydání zdroje, tedy řádově měsíc. **Ruční běh to opravit nemohl:** ukládá pod týž klíč a `cache/save` na obsazeném klíči nic nepřepíše. Krok přitom skončí úspěchem — nezhavaruje, ale ani neuloží; vypíše jen anotaci `Cache save failed.`, kterou GitHub zobrazí nad výpisem běhu. Ověřeno během `34350398991` (9. 9. 2026 12:20), který ukládal pod klíč obsazený týž den ráno. Komentář v `import-mesicni.yml` to zdůvodňoval tím, že „pod stejným klíčem jsou stejná data“ — to platí pro `nrpzs.csv` a `ruian_adr.zip`, ale ne pro meta soubor, který je v témže balíku. Tři opravy: `last_modified` má nově stejný odstup mezi pokusy jako `download` a `get_bytes` (sníží pravděpodobnost), **obě workflow cache neuloží, když je `nahradniDatum` neprázdné** (ruší uváznutí strukturálně — porucha pak nepřežije do dalšího běhu) a README popisuje obnovu smazáním cache (`gh cache delete`) pro případ, že už otrávená je. Retry ani druhá pojistka nenahrazují jedna druhou: retry řeší krátký výpadek, podmínka u `save` řeší i ten dlouhý. |
