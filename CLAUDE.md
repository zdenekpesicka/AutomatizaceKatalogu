...
| **Testy nad zpracováním běží před stahováním** | `tests/test_zpracovani.py`, 23 testů. V obou workflow běží hned po instalaci závislostí a před prvním sáhnutím na zdroje (konkrétně jako 4. krok: checkout, setup-python, `pip install`, testy). Pokrývají místa, kde už jednou vznikla chyba v datech nebo kam ji zdroje aktivně zvou: prohozené pořadí ve WKT z ÚZIS (3.1) včetně odmítnutí bodu mimo ČR, rozpad vícehodnotového `ZZ_obor_pece` (3.3), slučování registrací podle `portalId` včetně případu s koncovou mezerou (8.3), datum v klíči cache, kontrolu odkazu z ATOM feedu a odstup mezi pokusy ve `fetch.get_bytes` (`requests.get` i `time.sleep` jsou nahrazené, síť se nepoužívá). Nejsou to testy „pro pokrytí“ — každý odpovídá konkrétní vlastnosti zdroje nebo poruše, která už jednou nastala. Nesahají na síť ani na `_cache/`, proto mohou běžet ještě před stažením: když je rozbité zpracování, nemá smysl tahat 190 MB dat. `pytest` je v odděleném `import/requirements-dev.txt`, který k `requirements.txt` přidává jen jeho. **Obě workflow instalují `requirements-dev.txt`**, protože testy v nich běží — dělení souborů tedy neslouží CI, ale tomu, kdo si chce postavit katalog bez testovací závislosti. |
