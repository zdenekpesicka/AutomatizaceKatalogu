...
| Téma | Pravidlo |
|---|---|
| **Verze schématu** | Aktuální je **1.3.0**. Odsouhlasené s vývojářem bylo 1.0.0; 1.1.0 přidalo `sluzby[].zarizeni`, 1.2.0 `sluzby[].oboryPece`, 1.3.0 kategorii `poradenstvi`. Žádná z nich neodebrala pole ani nezměnila typ. **Rozšíření výčtu v `kategorie[]` ze čtyř hodnot na pět (1.3.0) se podle 4.2 hlásilo dopředu**, na rozdíl od přírůstkových polí v 1.1.0 a 1.2.0. Schéma leží v témže repozitáři jako data a odebírá se ze stejné adresy, takže příjemce dostane nový výčet spolu s daty. Web se čtyřmi záložkami se tím nerozbije — jen dokud záložka Poradenství nevznikne, nezobrazí 217 míst, která nemají jinou kategorii; zbylých 157 míst s poradenstvím se zobrazuje dál pod svou druhou kategorií. Po spuštění webu už by táž změna byla důvod k 2.0.0. Verze je na třech místech a musí sedět: `VERZE_SCHEMATU` v `build_katalog.py`, `verzeSchematu` v `meta.json` a v `katalog.json`. `data/ukazka.json` je zmrazený na 1.0.0 záměrně, neaktualizuje se. |
