...
| **Testy nad zpracováním běží před stahováním** | `tests/test_zpracovani.py`, 21 testů, první krok obou workflow. Pokrývají místa, kde už jednou vznikla chyba v datech nebo kam ji zdroje aktivně zvou: prohozené pořadí ve WKT z ÚZIS (3.1) včetně odmítnutí bodu mimo ČR, rozpad vícehodnotového `ZZ_obor_pece` (3.3), slučování registrací podle `portalId` včetně případu s koncovou mezerou (8.3), datum v klíči cache a kontrolu odkazu z ATOM feedu. Nejsou to testy „pro pokrytí“ — každý odpovídá konkrétní vlastnosti zdroje, kterou nelze odvodit z kódu. Nesahají na síť ani na `_cache/`, proto mohou běžet ještě před stažením: když je rozbité zpracování, nemá smysl tahat 190 MB dat. `pytest` je v odděleném `import/requirements-dev.txt`, aby produkční běh instaloval jen to, co ke stavbě katalogu potřebuje. |
| **Odolnost stahování a kontrola odkazu z ATOM feedu** | Tři úpravy z auditu 9. 9. 2026. **(a) Odstup mezi pokusy:** `fetch.py` opakoval pokusy bez čekání, takže všech pět proběhlo během milisekund — dřív, než stačil odeznít i ten nejkratší výpadek zdroje, a retry jen zopakoval tutéž chybu. Nově 2, 4, 8, 16 s se stropem 16, celkem pod 30 s, tedy bezpečně uvnitř třicetiminutového limitu jobu. **(b) Kontrola odkazu z feedu ČÚZK:** `<id>` je jen řetězec a bez kontroly by přesměrování nebo změna feedu poslaly stahování kamkoli. `resolve_download_url` proto pinuje host `vdp.cuzk.gov.cz`, vyžaduje HTTPS a tvar `RRRRMMDD_OB_ADR_csv.zip`, a při chybějícím `<entry>` nebo `<id>` selže se srozumitelnou hláškou místo tichého pádu na náhradní klíč cache. Z názvu souboru se totiž čte datum do klíče cache. **(c) Actions připnuté na commit SHA** místo pohyblivých značek `v4`/`v5`, aby přepsaná značka nemohla změnit, co v běhu importu poběží. |
