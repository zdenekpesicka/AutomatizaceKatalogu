...
Soubory jsou statické, staví se přímo z větve `main`, žádné API se neprovozuje. Aktuální verze schématu je **1.4.0**, uvedená v `meta.json` i v `katalog.json`. `data/ukazka.json` je zmrazená ilustrace k dokumentaci ve verzi 1.0.0, neaktualizuje se a neodebírá se.
